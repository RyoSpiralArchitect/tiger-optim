...
def test_summarize_param_groups_multi_key_sort_defaults(demo_param_groups):
    summary = summarize_param_groups(demo_param_groups, sort_by=["lr_scale", "tag"])
    tags = _extract_summary_tags(summary)
    assert tags == ["mlp", "bias", "norm"]


def test_summarize_param_groups_multi_key_custom_directions(demo_param_groups):
    summary = summarize_param_groups(
        demo_param_groups,
        sort_by=("lr_scale", "tag"),
