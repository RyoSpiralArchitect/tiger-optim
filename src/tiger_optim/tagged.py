...
class ParamTagAggregate:
    ...
    values observed across the contributing parameter groups, including the
    *effective* learning rate derived from ``lr * lr_scale``. These extrema are
...
def aggregate_param_group_stats(
    ...
    for tag in order:
        ...
        if weight:
            ...
            avg_effective_lr = bucket["weighted_effective_lr"] / weight
        else:
            ...
            avg_effective_lr = bucket["sum_effective_lr"] / groups if groups else 0.0
        aggregates.append(
            ParamTagAggregate(
                tag=tag,
